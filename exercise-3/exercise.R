...
usp.data <- data.frame(USPersonalExpenditure)

# The variable USPersonalExpenditure is now accessible to you. Unfortunately,
# it's not a data frame (it's actually what is called a matrix)
# Test this using the `is.data.frame()` function
is.data.frame(usp.data)
...
category <- rownames(usp.data)


# How much money was spent on personal care in 1940?
money <- usp.data['Personal Care', 'X1940']
View(usp.data)

# How much money was spent on Food and Tobacco in 1960?
money.1960 <- usp.data['Food and Tobacco', 'X1960']


# What was the highest expenditure category in 1960?
# Hint: use the `max()` function to find the largest, then compare that to the column
highest <- usp.data$category[usp.data$X1960 == max(usp.data$X1960)]
